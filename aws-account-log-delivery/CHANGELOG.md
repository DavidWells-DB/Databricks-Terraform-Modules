...
### Fixed
- **`BILLABLE_USAGE` log delivery failed** — `output_format` was hardcoded to `JSON`, but the API rejects JSON for `BILLABLE_USAGE` (only `CSV` is supported). Now selects `CSV` for `BILLABLE_USAGE` and `JSON` for `AUDIT_LOGS`.
